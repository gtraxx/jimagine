...
(function($){
    ...
    function _setBoxParams(setting){
        ...
        if(setting != null){
            ...
            if(setting.elemid == undefined){
                set_elemid = optDefault.elemid;
            }else{
                set_elemid = setting.elemid;
            }
            if(setting.elemclass == undefined){
	...
	$.nicenotify.initbox = function(request,setDefault,setNotifier) {
		...
		if ( !$.isPlainObject( opts ) ){
			console.log( optsNotifier.elemid );
		}
		if(typeof opts != 'object'){
			console.log('nicenotify default initbox is not object');
		}else if(typeof optsNotifier != 'object'){
            console.log('nicenotify notifier initbox is not object');
        }
	    if(opts.display != false){
	    	var $id = $(optsNotifier.elemid);
            switch(optsNotifier.box){
                case 'meerkat':
                    if(jQuery().meerkat){
                        $id.destroyMeerkat();
                        $id.meerkat({
            ...
            if(opts.text != false || opts.text != '' || opts.text !=null){
